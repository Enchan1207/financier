...
export const getUserById = (db: D1Database) => async (id: string): Promise<User | undefined> => {
  const stmt = d1(db)
    .select(UserRecord, 'users')
    .where(condition('id', '==', id))
    .build()

  return stmt.first<UserRecord>().then(item => item === null ? undefined : makeEntity(item))
}

export const getUserByAuth0Id = (db: D1Database) => async (id: string): Promise<User | undefined> => {
  const stmt = d1(db)
    .select(UserRecord, 'users')
...
export const saveUser = (db: D1Database) => async (newUser: User): Promise<User> => {
...
export const fetchUserInfo = (authDomain: string) => async (token: string): Promise<Auth0UserInfo | undefined> => {
