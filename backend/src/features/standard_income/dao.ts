...
import type {
  StandardIncomeGrade, StandardIncomeTable, StandardIncomeTableSummary,
...
const makeRecord = (entity: StandardIncomeTable): [StandardIncomeTableRecord, StandardIncomeGradeRecord[]] => [{
  id: entity.id,
...
/** 新しい標準報酬月額表を追加 */
export const insertStandardIncomeTable = (db: D1Database):
(item: StandardIncomeTable) => Promise<StandardIncomeTable> =>
  async (entity) => {
    const [tableRecord, gradeRecords] = makeRecord(entity)

    const tableInsertStmt = 'INSERT INTO standard_income_tables VALUES (?1,?2,?3)'
    const tableInsertQuery = db.prepare(tableInsertStmt).bind(
      tableRecord.id,
      tableRecord.user_id,
      tableRecord.name,
    )

    const gradeInsertStmt = 'INSERT INTO standard_income_grades VALUES (?1,?2,?3)'
    const gradeInsertQueries = gradeRecords.map(grade => db
      .prepare(gradeInsertStmt).bind(
        tableRecord.id,
        grade.threshold,
        grade.standard_income,
      ))
...
/** 標準報酬月額表の名前を変更する */
export const updateStandardIncomeTableName = (db: D1Database):
(props: {
  userId: User['id']
  ...
  async (props) => {
    const updateQueryBase = 'UPDATE standard_income_tables SET name=? WHERE id=? AND user_id=?'
...
/** 標準報酬月額表の階級を変更する */
export const updateStandardIncomeTableGrades = (db: D1Database):
(props: {
  userId: User['id']
  id: StandardIncomeTable['id']
  ...
  async (props) => {
    const gradeRecords: StandardIncomeGradeRecord[] = props.grades.map(grade => ({
    ...
    const gradeInsertStmt = 'INSERT INTO standard_income_grades VALUES (?1,?2,?3)'
    const gradeInsertQueries = gradeRecords.map(record => db
      .prepare(gradeInsertStmt).bind(
        record.income_table_id,
        record.threshold,
...
export const listStandardIncomeTables = (db: D1Database):
(props: {
  userId: User['id']
  order?: 'asc' | 'desc'
...
/** IDを指定して標準報酬月額表を取得する */
export const getStandardIncomeTable = (db: D1Database):
(props: {
  userId: User['id']
  id: StandardIncomeTable['id']
  ...
  async (props) => {
    ...
    const [{ results: [tableRecordRaw] }, { results: gradesRecordRaw }]
    = await db.batch(queries)

    const tableRecord = StandardIncomeTableRecord.safeParse(tableRecordRaw)
    const gradeRecords = z.array(StandardIncomeGradeRecord)
