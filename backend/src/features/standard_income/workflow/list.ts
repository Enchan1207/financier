...
type ListStandardIncomeTablesSchema = z.infer<
  typeof ListStandardIncomeTablesSchema
>
...
const listStandardIncomeTables =
  ...
  async (
    command: UnvalidatedListStandardIncomeTablesCommand,
  ): Promise<StandardIncomeTableSummary[]> => {
    const {
      input: { order },
      state: {
...
type ListStandardIncomeTablesWorkflow = (
  command: UnvalidatedListStandardIncomeTablesCommand,
) => Promise<StandardIncomeTableSummary[]>
