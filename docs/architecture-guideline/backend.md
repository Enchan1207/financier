...
- 更新系はワークフローを呼び出す
- **コマンド構築はデータの受け渡しのみ**: リクエストボディの値をコマンドの各フィールドに直接代入する。値の有無を判定する条件スプレッド（`...(x !== undefined ? { k: x } : {})`）など、データ変換・判定ロジックを route に書いてはならない
  - オプショナルフィールドで `undefined` と他の値（`null` など）を区別したい場合は、コマンド型のプロパティを `?: T | undefined` と定義し、route は `key: body.key` と渡すだけにする
...
7. ❌ コマンド構築時に条件スプレッドやデータ変換ロジックをrouteに記述（workflowのコマンド型で表現する）
