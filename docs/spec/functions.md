...
**拠出停止の実装方針**: completed 状態の積立への拠出は、UI レベルで制御する。紐づくカテゴリへの支出トランザクション登録時に、そのカテゴリに紐づく積立定義が completed または archived の場合、警告を表示し、通常カテゴリへの変更を促す。ただし、技術的には登録自体は可能とする（強制禁止はしない）。
