...
- 4a. 適切なカテゴリが存在しない場合 → UC-3.1（カテゴリ作成）を実行後、再開
...
- 対象トランザクションが属する年度が active 状態である
...
| 未来年度への大量取引登録       | 許容する。内部残高には当該日到達まで含めない         |
| イベント削除時の取引処理       | 紐づくトランザクションのイベント紐付けを解除する     |
