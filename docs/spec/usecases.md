...
- 3a. 取引種別とカテゴリ種別が不一致 → 保存を拒否し、種別不一致エラーを表示
...
- 一括登録対象の各取引について、取引種別（`Transaction.type`）とカテゴリ種別（`Category.type`）が一致している
...
- 5a. 一括登録対象に取引種別とカテゴリ種別の不一致が含まれる場合 → 一括保存を拒否し、種別不一致エラーを表示
