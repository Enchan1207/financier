...
収入または支出の記録単位。日々の収支はすべてトランザクションとして記録される。各トランザクションには内容を説明する名前を必須で持たせる。
...
テンプレート取引は積立定義非紐付けカテゴリのみを対象とする。取引種別（`Transaction.type`）はテンプレートで直接指定せず、保存時にカテゴリ種別（`Category.type`）から決定する。
