...
```
routes/<feature>/
├── index.tsx           # ページコンポーネント（モックデータをインラインで定義）
└── -components/        # ページ固有コンポーネント（ダイアログ・カードなど）
    └── <name>.tsx
```

`-repositories/` は API 統合後に追加するため、モック段階では不要。
...
// 行クリックでダイアログを開く
<TableRow onClick={() => { setTarget(row); setEditOpen(true) }}>

// ダイアログに渡す（モックのため onSave はローカル状態を更新するだけ）
<EditDialog
  open={editOpen}
...
```typescript
// -components/<entity>-edit-dialog.tsx
type Props = {
  open: boolean
...
export const CategoryEditDialog: React.FC<Props> = ({ open, onOpenChange, target, onSave }) => {
  const form = useForm({
...
既存の `routes/savings/new/index.tsx` や `routes/event-templates/` を参照。

```typescript
// Zod スキーマで検証ルールを定義
const schema = z.object({
  name: z.string().min(1, '名前を入力してください'),
  amount: z.string().refine((v) => parseInt(v, 10) > 0, '金額は1以上にしてください'),
})

// useForm で初期化（モックのため onSubmit は非同期でなくてよい）
const form = useForm({
...
`frontend/components/layout/nav-items.ts` の `navGroups` に追記するだけでよい。`to` は `FileRouteTypes['to']` 型なので、ルートファイルを先に作成しておくこと。
