...
### 6. event-templates/$id/register/index.tsx の追加改善 ✅ 完了

`useTemplateForm` 系の移行完了後、一括登録画面（`register/index.tsx`）についても以下の改善を追加で実施した。

| 改善項目     | 実装内容                                                                                            |
| ------------ | --------------------------------------------------------------------------------------------------- |
| useForm 移行 | `useState` を廃止し `useForm` で `date` と `items[].amount` を管理するよう変更                      |
| DatePicker   | 「取引日（全件共通）」フィールドを Calendar/Popover で実装、初期値は `dayjs().format('YYYY-MM-DD')` |
| 1秒待機      | `onSubmit` に `await new Promise(resolve => setTimeout(resolve, 1000))` を追加                      |
| Spinner      | 「一括保存」ボタンに `Loader2Icon` スピナーを追加、`isSubmitting` 中に表示                          |
